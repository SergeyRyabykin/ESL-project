...
#define SIZE_IN_WORDS(data) (sizeof(data) / sizeof(uint32_t) + ((sizeof(data) % sizeof(uint32_t)) ? 1 : 0))
#define CUSTOM_NVM_RECORD_SIZE_IN_WORDS(payload_size) (SIZE_IN_WORDS(custom_nvm_record_info_t) + SIZE_IN_WORDS(payload_size))
...
static ret_code_t custom_nvm_write_record(const void *object, const size_t object_size, const custom_nvm_payload_id_t id, const uintptr_t addr)
{
    ...
    memcpy(&record_array[SIZE_IN_WORDS(info)], object, object_size);
...
uintptr_t custom_nvm_find(const custom_nvm_payload_id_t id)
{
    ...
    if(record) {
        return record + SIZE_IN_WORDS(custom_nvm_record_info_t) * sizeof(uint32_t);
    }
