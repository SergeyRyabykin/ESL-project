...
volatile bool is_complited = false;

void custom_fstorage_cb(nrf_fstorage_evt_t * p_evt)
{
    switch (p_evt->id) {
        case NRF_FSTORAGE_EVT_WRITE_RESULT:
            NRF_LOG_INFO("Fstorage EVT written");
            break;
        case NRF_FSTORAGE_EVT_ERASE_RESULT:
            NRF_LOG_INFO("Fstorage EVT erased");
            break;
        default:
            NRF_LOG_INFO("Fstorage EVT fef: %x", p_evt->id);
            break;
    }
...
NRF_FSTORAGE_DEF(nrf_fstorage_t g_custom_storage) = 
{
    .evt_handler = custom_fstorage_cb,
    .start_addr = 0xe0000 - NRF_DFU_APP_DATA_AREA_SIZE,
    .end_addr = 0xe0000,
};
...
//         custom_nvm_record_info_t record;
//         if(NRF_SUCCESS == nrf_fstorage_read(&g_custom_storage, CUSTOM_NVM_APP_DATA_PAGE_ADDR(i), &record, sizeof(custom_nvm_record_info_t))) {
...
static ret_code_t custom_nvm_write_record(const void *object, const size_t object_size, const custom_nvm_payload_id_t id, const uintptr_t addr)
{
    ...
    ret_code_t ret = nrf_fstorage_write(&g_custom_storage, addr, record_array, record_size_w * 4, NULL);
    ...
    //         ret_code_t ret = nrf_fstorage_write(&g_custom_storage, current_word_addr, &record_array[i], sizeof(record_array[0]), NULL);
    //         if(NRF_SUCCESS == ret) {
    //             wait_for_complete();
    //         }
    //         else {
            ...
            if(custom_nvm_is_addr_valid_for_page(dest_addr, dest_page, record->payload_size)) {
                ...
                for(int i = record_size_w - 1; i >= 0 ; i--){
                    ...
                    if(nrfx_nvmc_word_writable_check(current_word_addr, *((uint32_t *)record + i))) {
                        ...
                        ret_code_t ret = nrf_fstorage_write(&g_custom_storage, current_word_addr, (uint32_t *)record + i, sizeof(uint32_t), NULL);
                        if(NRF_SUCCESS == ret) {
                            wait_for_complete();
                        }
                        else {
...
ret_code_t custom_nvm_erase(void) {
    ...
    ret_code_t ret = nrf_fstorage_erase(&g_custom_storage, CUSTOM_NVM_APP_DATA_PAGE_ADDR(0), 3, NULL);
    ...
    if(NRF_SUCCESS != ret) {
        ...
        while(NRF_SUCCESS != ret && page != next_page) {
            ...
            if(NRF_SUCCESS == ret) {
                ...
                ret = nrf_fstorage_erase(&g_custom_storage, CUSTOM_NVM_APP_DATA_PAGE_ADDR(page), 1, NULL);
